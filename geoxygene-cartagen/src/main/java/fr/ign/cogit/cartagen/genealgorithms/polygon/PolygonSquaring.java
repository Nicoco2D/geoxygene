...
public class PolygonSquaring {
	...
	private void simpleSquaringInit() {
		...
		for (int i = 0; i < this.nb_edges; ++i) {
			...
			if (logger.isTraceEnabled())
				logger.trace("vec" + i + " (" + vecs[i].getX() + " ; " + vecs[i].getY());
		...
		angles[0] = Math.PI - vecs[vecs.length - 1].angleVecteur(vecs[0]).getValeur();
		for (int i = 0; i < vecs.length - 1; ++i) {
			angles[i + 1] = Math.PI - vecs[i].angleVecteur(vecs[i + 1]).getValeur();
			align[i] = Math.abs(vecs[i].prodVectoriel(vecs[primaryAxe]).getZ());
		}
		// this.printAngles();
		align[align.length - 1] = Math.abs(vecs[align.length - 1].prodVectoriel(vecs[primaryAxe]).getZ());
	}
	...
	private Vecteur rotateVec(int deg, int vecToMove, int vecFixed, int i) {
		...
		if (deg == 90) {
			...
			logger.trace("vtomov : " + vecs[vecToMove].getX() + " ; " + vecs[vecToMove].getY());
			return vSquared;
		}
		if (deg == 45) {
			// x' = x.cos pi/4 - y.sin pi/4 -- cos Pi/4=0.7071067811865476=sin
			// pi/4
	...
	public IPolygon simpleSquaring() {
		...
		for (int i = 0; i < angles.length; ++i) {
			if (Math.abs((Math.PI - angles[i])) <= this.angTol && Math.abs((Math.PI - angles[i])) > correctTol) {
				int[] v = getVecsAroundPoint(i);
				Vecteur vv = vecs[v[0]].ajoute(vecs[v[1]]);
				vv.normalise();
				double norme = vecs[v[0]].norme() * Math.cos(vecs[v[0]].angleVecteur(vv).getValeur());
				Vecteur vSquared = vv.multConstante(norme);
				IDirectPosition p = new DirectPosition(points.get(v[0]).getX(), points.get(v[0]).getY());
				p.setCoordinate(p.getX() + vSquared.getX(), p.getY() + vSquared.getY());
				points.set(i, p);
				update();
				logger.trace("Angle " + i + " now flattened " + angles[i] * 180 / Math.PI);
			}
		}
		...
		for (int i = 0; i < angles.length; ++i) {
			logger.debug("Testing point " + i + " -- Angle " + angles[i] * 180 / Math.PI);
			...
			if (Math.abs((Math.PI / 2 - angles[i])) <= this.angTol
				...
				IDirectPosition p = new DirectPosition(points.get(i).getX(), points.get(i).getY());
				p.setCoordinate(p.getX() + vSquared.getX(), p.getY() + vSquared.getY());
				points.set(pointToMove, p);

				update();
				logger.trace("Angle " + i + " now squared " + angles[i] * 180 / Math.PI);
			} // testing pi/4 candidates
			else if (Math.abs((Math.PI / 4 - angles[i])) <= this.angTol && Math.abs((Math.PI / 4 - angles[i])) > 0.01) {
				int[] v = getVecsAroundPoint(i);
				int vecToMove = v[0], vecFixed = v[1];
				if (align[vecToMove] < align[vecFixed]) {
					vecToMove = v[1];
				...
				boolean angleBeforeIsRight = Math.abs(angles[angleBefore] - angles[angleBefore]) < correctTol;
				boolean angleAfterIsRight = Math.abs(angles[angleAfter] - angles[angleAfter]) < correctTol;
				...
				points.set(pointToMove, p);
				update();
				logger.trace("Angle " + i + " now half squared " + angles[i] * 180 / Math.PI);
			}
		}
